...
static int fputf(FILE *fd, const char *fmt, ...)
{
        char __sys[1024];
        int ret_code;
        va_list args;
...
#if 0
		/*
		 * This block is #if 0'd for an OBVIOUS reason.
		 * If you see this in the future, I'm not to blame
		 * for this.
		 *
...
//        fputs( "; Set the height of the window\ny=200\n\n", fd );
		...
		fputf(fd, "; Abuse-SDL Configuration file\n\n");
		fputf(fd, "; Startup fullscreen\nfullscreen=%i\n\n", flags.fullscreen);
		fputf(fd, "; Gamma Correction\ngamma=%i\n\n", flags.gamma);
#ifdef __APPLE__
		fputf(fd, "; Use DoubleBuffering\ndoublebuf=%i\n\n", flags.doublebuf);
		fputf(fd, "; Use OpenGL\ngl=%i\n\n", flags.gl);
#else
		fputf(fd, "; Use DoubleBuffering\ndoublebuf=%i\n\n", flags.doublebuf);
		fputf(fd, "; Use OpenGL\ngl=%i\n\n", flags.gl);
		fputf(fd, "; Location of the datafiles\ndatadir=");
		fputf(fd,  ASSETDIR "\n\n");
#endif
		fputf(fd, "; Use mono audio only\nmono=%i\n\n", flags.mono);
		fputf(fd, "; Grab the mouse to the window\ngrabmouse=%i\n\n", flags.grabmouse);
		fputf(fd, "; Set the scale factor\nscale=%i\n\n", scale);
		fputf(fd, "; Use anti-aliasing (with gl=1 only)\nantialias=%i\n\n", flags.antialias);
        fputf(fd, "; Set the width of the window\nx=%i\n\n", flags.xres);
        fputf(fd, "; Set the height of the window\ny=%i\n\n", flags.yres);
		fputf(fd, "; Disable the SDL parachute in the case of a crash\nnosdlparachute=%i\n\n", flags.nosdlparachute);
		fputf(fd, "; Key mappings\n");

		char buffer[32];

		key_name(keys.left, buffer);
		fputf(fd, "left=%s\n", buffer);
		key_name(keys.right, buffer);
		fputf(fd, "right=%s\n", buffer);
		key_name(keys.up, buffer);
		fputf(fd, "up=%s\n", buffer);
		key_name(keys.down, buffer);
		fputf(fd, "down=%s\n", buffer);

		key_name(keys.b2, buffer);
		fputf(fd, "fire=%s\n", buffer);
		key_name(keys.b1, buffer);
		fputf(fd, "special=%s\n", buffer);
		key_name(keys.b3, buffer);
		fputf(fd, "weapprev=%s\n", buffer);
		key_name(keys.b4, buffer);
		fputf(fd, "weapnext=%s\n", buffer);
...
void readRCFile() {
	...
	if ((fd = fopen(rcfile, "r")) != NULL) {
		while (fgets(buf, sizeof(buf), fd) != NULL) {
			...
			} else if (strcasecmp(result, "gamma") == 0) {
				result = strtok(NULL, "\n");
				flags.gamma = atoi(result);
...
void parseCommandLine(int argc, char **argv) {
	for (int ii = 1; ii < argc; ii++) {
		...
		} else if (!strcasecmp(argv[ii], "-h")
				|| !strcasecmp(argv[ii], "--help")) {
			showHelp();
			exit(0);
		}
	}
...
void setup(int argc, char **argv) {
	...
	flags.yres = yres = 240; // Default window height
	flags.gamma = 5;
#ifdef __APPLE__
